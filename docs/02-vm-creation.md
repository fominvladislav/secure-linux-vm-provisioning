...
- host firewall настроен и проверен, если он обязателен по принятой security-policy;
- SELinux/AppArmor находится в требуемом состоянии;
- системное логирование работает;
- security agents подключены и проверены, если они обязательны;
- vulnerability/security check выполнен, если он обязателен;
- отсутствуют незакрытые блокирующие security findings.

Если обязательное для данной VM требование не выполнено, продолжать runbook нельзя.
...
## 13. Monitoring, backup и CMDB

До начала этого этапа на стадии Preparation должны быть определены конкретные monitoring, backup и CMDB-механизмы.

Абстрактные значения вроде `monitoring-agent` не должны использоваться как реальные имена сервисов без адаптации под инфраструктуру.

### Monitoring

Установить и настроить согласованный monitoring-agent в соответствии с корпоративной инструкцией.

Если monitoring-agent работает как systemd service, проверить его состояние:

```bash
sudo systemctl status <monitoring-service> --no-pager
sudo systemctl is-active <monitoring-service>
```
<monitoring-service> необходимо заменить на реальное имя сервиса.

Локально запущенного агента недостаточно.

На стороне monitoring-системы необходимо убедиться, что:

- VM появилась под ожидаемым hostname;
- поступают актуальные метрики;
- применены согласованные шаблоны и теги;
- VM находится в правильной группе;
- test alert успешно сформирован и доставлен в ожидаемый канал.

Если метрики не поступают или test alert не доставляется, этап Monitoring не считается завершенным.

### Backup

Подключить VM к согласованной backup policy/job.

После подключения выполнить тестовый backup job.

На стороне backup-системы необходимо убедиться, что:

- job завершился успешно;
- резервируется ожидаемая VM или согласованный перечень данных;
- используется правильная backup policy;
- schedule соответствует требованиям;
- retention соответствует требованиям;
- RPO/RTO соответствуют подготовленным требованиям.

После успешного backup выполнить согласованную проверку восстановления.

Для файлового backup это может быть восстановление тестового файла в временное расположение с последующей проверкой его содержимого.

Для image-level backup использовать предусмотренную корпоративной системой процедуру test restore или recovery verification.

Backup не считается проверенным только по факту успешного создания job.

Snapshot гипервизора не является заменой полноценного backup.

Если test backup или проверка восстановления не завершились успешно, продолжать приемку нельзя.

### CMDB

Зарегистрировать или обновить VM в используемой CMDB.

Как минимум проверить наличие согласованных для инфраструктуры данных:
...
- владелец;
- ОС;
- ответственные/support group;
- информация о monitoring;
- информация о backup.

После регистрации убедиться, что объект VM существует в CMDB и имеет ожидаемый идентификатор или CI.

Если по внутреннему процессу CMDB обязательна, но объект не зарегистрирован, продолжать приемку нельзя.

### Exit criteria

Этап считается завершенным только если:

- monitoring подключен и реальные метрики поступают;
- test alert успешно проверен;
- test backup job завершился успешно;
- выполнена согласованная проверка восстановления;
- backup policy и retention соответствуют требованиям;
- CMDB зарегистрирована или обновлена, если она обязательна.

Формулировка «подключение запланировано» не считается успешным завершением этого этапа.
...
VM может быть переведена в состояние READY только если выполнены все обязательные для нее проверки. 

VM готова, когда:

- ОС корректно загружается; 
- hostname и FQDN соответствуют подготовленным данным;
- сеть работает; - присутствуют только ожидаемые маршруты;
- прямой и обратный DNS работают; - время синхронизировано;
- необходимые обновления установлены; - Basic security успешно пройден;
- все запрошенные дополнительные filesystem настроены и автоматически монтируются после reboot;
- постоянный административный доступ проверен;
- временная учетная запись `bootstrap-admin` удалена вместе с ее sudo-правами;
- domain integration успешно проверена, если она требуется; - monitoring подключен, метрики поступают и test alert проверен;
- backup подключен, test backup job и проверка восстановления выполнены успешно;
- VM зарегистрирована в CMDB, если это обязательное требование;
- доступ Linux Operations подтвержден;
- automated validation завершилась с exit code `0`.

Состояния `FAIL` и `INCOMPLETE`, незавершенные обязательные проверки или формулировки «будет настроено позже» не допускают перевод VM в READY. Исключение возможно только если оно официально согласовано внутренним процессом и имеет ответственного и срок устранения.
